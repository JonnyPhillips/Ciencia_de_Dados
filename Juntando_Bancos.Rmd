...
2. Execute um join dos bancos de dados `flights` e `planes` para apenas os voos de `carrier` United (UA) no dia 16 de setembro de 2013. Qual é o modelo `model` mais comum destes voos?
...
# Bancos e Dados Faltantes (`right_join`, `inner_join`, `outer_join`, `anti_join`)
...
1. Crie dois tibbles pequenos (com a função `tibble()`) como eles abaixo e juntar eles pelos identificadores comuns. 

```{r, echo=F}
t1 <- tibble(ID=c("A001","A001","A002","A003","A003"),
             Ano=c(2019, 2020, 2020, 2019, 2020),
             Valor=c(10, 12, 17, 50, 64)) %>% paged_table()
t2 <- tibble(ID=c("A001","A002","A002","A003"),
             Ano=c(2019, 2019, 2020, 2019),
             População=c(3000, 2500, 1900, 6000)) %>% paged_table()
t1
t2
```
...
3. Usando um join, identifique a precipitatação (`precip`) média no momento de partido dos voos de LGA em cada dia de dezembro.
...
(a) Usando um join apropriado, gere um banco de dados com a visibilidade em cada hora do ano e o número de voos.

(b) Resumo o seu banco de dados para estimar o número de voos por hora média por valor da variável visibilidade. Mostre os resultados num gráfico de pontos.
...
weather %>% left_join(num_voos_per_hora, by=c("origin", "year", "month", "day", "hour")) %>% group_by(visib) %>%
...
1. Crie um tibble nested, com a unidade de análise cada origem-destino par, e a coluna de dados todos os detalhes dos voos entre aquele origem e destino.

```{r, echo=F, eval=F}
flights %>% group_by(origin, dest) %>% nest()
```

2. Crie um tibble nested por aeroporto de origem, hora, dia e mês. Use um join para juntar os dados de `weather`.
...
Antes da próxima aula, por favor leia [R 4 Data Science, Capítulo ](https://r4ds.had.co.nz)
