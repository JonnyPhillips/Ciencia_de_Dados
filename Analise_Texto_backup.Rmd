...
2. No mesmo banco, Substitua os nomes das companhias aéreas que contém 'Inc.' com 'Incorporated', e eles que contém 'Co.' com 'Company'. 
...
# Comparando Documentos (`bind_tf_idf`)

É possível trabalhar além de uma descrição de um livro único e *comparar* o conteúdo de documentos distintos. Para isso, precisamos distinguir *documentos* (livros ou artigos distintos) e *termos* (palavras, ngrams etc. em um documento). Fazemos isso em um matriz de documento-termo, em que cada documento é uma linha e cada termo é uma coluna. 

Primeiramente, temos que abrir mais um documento. Vamos pegar o livro português de Jules Verne, 'Da terra á lua' de Projeto Gutenberg:
...
E vamos processar o banco de dados para padronizar o conteúdo e gerar os stems:
...
Agora precisamos juntar os dois bancos, 'Assis_palavras' e 'Verne_palavras' e um tibble, com uma coluna identificado o documento relevante. Para deixar os resultados mais fácil a interpretar, vamos trabalhar com as palavras mesmas em vez dos stems:
...
E finalmente, transformamos o tibble em um matriz de documento-termo com a função `cast_dtm()`, indicando como argumentos a coluna que identifica o documento, a coluna que contém as palavras (termos), e a coluna que conta a frequência de cada palavra.
...
<!-- More to compare docs with simple dtm -->
...
Quais palavras distinguem o melhor os dois livros? Ou seja, há palavras frequentes que existem nos dois livros (palavras comuns), e há palavras que são específicas para cada livro. A medida que é usada na análise de texto é a frequência do termo ('tf') multiplicado pelo frequência inversa do termo no documento ('idf') - o 'tf-idf'. Os termos com os maiores tf-idf são os termos que são mais presentes em um documento *e não no outro*. 

Executamos este análise com o tibble dos dois livros, 'Assis_verne', e a função `bind_tf_idf()`. 
...
Com o nosso matriz de documento-termos, usamos a função `LDA()` do pacote `topicmodels` para classificar o conteúdo textual automaticamente. Observe que o modelo não vai utilizar a informação do documento - ele só vai agrupar os termos em aglomerados com temas parecidas. Vamos especificar dois tópicos para tentar recuperar uma distinção entre os dois livros.

```{r}
#install.packages("topicmodels")
library(topicmodels)

Assis_Verne_LDA <- LDA(Assis_Verne_dtm, 2)
```

O resultado de `LDA()` pode ser transformado em nosso amigo, o tibble, com a função `tidy()` do pacote `broom()`, o mesmo que usamos com regressões.

```{r}
library(broom)

Assis_Verne_LDA_tidy <- Assis_Verne_LDA %>% tidy()
```

```{r, eval=F}
Assis_Verne_LDA_tidy
```

```{r, echo=F}
Assis_Verne_LDA_tidy %>% slice(1:10) %>%
  kable()
```


Observe que cada linha em 'Assis_Verne_LDA_tidy' é duplicado agora - uma vez por cada tópico, com um 'beta' associado. O 'beta' reflete a probabilidade que o termo é associado com o tópico. 

```{r, eval=F}
Assis_Verne_LDA_tidy %>% group_by(topic) %>% 
  top_n(5, beta) %>%
  arrange(-beta)
```
...
Podemos visualizar os termos mais frequentes por tópico:
...
Os resultados nào são perfeitos, mas é possível observar as palavras do livro de Jules Verne na esquerda: 'barbicane' (uma personagem central), 'lua' e 'mil'. Podemos verificar a connexão entre os tópicos e os documentos:

```{r, eval=F}
Assis_Verne_LDA_tidy_gamma <- Assis_Verne_LDA %>% tidy(matrix='gamma')

Assis_Verne_LDA_tidy_gamma
```

```{r, echo=F}
Assis_Verne_LDA_tidy_gamma <- Assis_Verne_LDA %>% tidy(matrix='gamma')

Assis_Verne_LDA_tidy_gamma %>% kable()
```


A tabela indica que o livro de Verne foi associado com alta probabilidade com tópico 1, e o livre de Assis com tópico 2. Parece que o modelo teve um grau de sucesso em classificar os dois livros corretamente. 
...
3. Junte dois textos tokenizados em um tibble com uma coluna que diferencia os fontes e uma coluna que conta a frequência de cada palavra: o texto que você identificou acima em questão 1, e o livro que você usou em exercício 2.
...
4. Calcule a medida 'tf_idf' para identificar os 5 palavras mais distintas para cada documento.
...
O prazo para entregar Desafio 5 por email com título “[FLS6397] - D5” à minha conta é **14h, 26/06/2020**, antes da aula. Por favor entregue (i) o arquivo .Rmd (ou .Rnw se preferir), e (ii) o arquivo .html ou .PDF.
</div>