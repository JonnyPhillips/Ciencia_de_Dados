...
1. Quantos voos decolaram de Nova Iorque em cada mes de 2013?
...
O que acontece se usarmos o tibble agrupado em uma nova operação? Imaginar que em mais três páginas queremos calcular a média de atrasos (`dep_delay`) com `summarize`?
...
Recebemos três linhas, mesmo que pedimos só uma média! O agrupamento ainda aplica para restringir o escope do resumo de `summarize`, e o R gerou uma média por aeroporto (`origin`). Mais geralmente, se esquecemos do agrupamento, é fácil gerar o resultado inesperado.
...
Fácil de fazer, difícil de lembrar! Presta atenção!


## Porcentagens

Um dos pedidos mais comuns é calcular porcentagens em R. Tome cuidado: já vi muitos cálculos errados, mesmo sendo porcentagem uma variável simples. A chave para evitar erros está na definição do *denominador* da fórmula da porcentagem:

$$\text{%} = \frac{\text{Valor}}{\text{Total do grupo relevante}}*100 $$

Queremos uma porcentagem para cada observação no banco de dados, então isso exige um `mutate()` Por exemplo, se quisemos calcular a porcentagem da distância de cada voo na distância total de todos os voos:
...
```{r, echo=F, eval=F}
flights %>% group_by(dist_total=sum(distance,na.rm=T))
```
...
5. Transforme as variáveis `dep_time`, `sched_dep_time`, `dep_delay`, `arr_time`, `sched_arr_time` e `arr_delay` de mensuração em minutos para mensuração em horas. Escreve uma função nova para facilitar esta transformação.
