...
3. Calcule o atraso média por aeroporto de origem (`dep_delay`). Qual aeroporto tem o pior atraso?
...
Se quisermos a porcentagem da distância de cada voo no total de cada mês, temos que limitar o escopode `sum()` apenas para as observações de um mesmo mês. O `group_by()` facilita isso:
...
A terceira opção é a mais eficiente e vai identificar automaticamente as colunas numéricas - esta variedade final de `mutate`, `mutate_if()` aceita um 'teste' em vez de uma lista de variáveis para serem transformadas. O teste é uma função simples que devolve o resultado `TRUE` ou `FALSE` para cada coluna. Em nosso caso, queremos testar se a coluna é numérica, então vamos usar o teste (a função) `is.numeric()`:
...
5. Transforme a ordem de grandeza de todas as variáveis `dep_delay`, `arr_delay` e `air_time` de minutos para horas. Escreva uma função nova para facilitar esta transformação.
