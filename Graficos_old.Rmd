...
Poderíamos. Mas mais interessante é compara as distribuições em um mesmo gráfico. Para fazer isso, precisamos saber como visualizar duas variáveis do nosso tibble ao mesmo tempo. Como estamos separando uma distribuição de uma variável contínua (`dep_time`) em três, a partir de uma segunda variável discreta (`origin`), precisamos adicionar essa nova variável à "aesthetics". Veja como:
...
O que signifíca o `~1` aqui? Em R o `~` significa uma fórmula, que tem dois lados, a esquerda do `~` e a direita. Em `facet_grid` a fórmula está usada para dividir o vertical, na esquerda, e o horizontal, na direita. O `1` significa um constante, ou seja que não queremos subdividir os dados em gráficos distintos nesta dimensão. O valor disso é que podemos gerar uma grade de gráficos, por uma variável no horizontal e outro no vertical:
