...
Note que temos que escrever o nome do objeto para que o resultado aparece no documento final. Se não, ele fica salvado no objeto 'area' para uso no futuro.
...
2. Selecione a linha de código dentro do seu script (na verdade apenas precisa deixar o cursor na linha relevante) e toque 'Run' em R ou digite `Ctrl+Enter`.
...
Note que a execução do código por qualquer método gerou o objeto `area` na janela/aba de 'Environment' em RStudio.  O Environment armazena todos os objetos que são disponíveis em modo interativo.
...
O pacote `nycflights13` contém um banco de dados se chama `flights`. É literalmente um banco de dados de todos os voos que partiam de Nova Iorque em 2013.
...
Olhanda na tabela de `flights`, e antes de avançar, responde as seguintes perguntas:
