...
Vamos tentar mais uma análise. Por exemplo, calculamos a área de um círculo de raio 20 com o chunk abaixo, usando a constante `pi`: 
...
Equações (na sintaxe do Latex) | Tabelas criadas com código de R
-                              | Gráficos criadas com código de R

# Formatação de Texto em R Markdown

Em contraste de Microsoft Word, formatar textos em R Markdown não depende de clicar em botões - precisamos incluir caracteres/símbolos no script para indicar a formatação que queremos. 
...
## Equações
Podemos escrever equações com a linguagem de Latex (que vamos aprender na semana que vem). Se quiser inserir uma equação dentro de uma frase (na mesma linha), use `$ equation $`, e para centralizar numa nova linha use `$$ big equation $$`. A sintaxe é assim:
...
Como incluimos o resultado numérico `area` do cálculo acima dentro de uma frase em nosso texto (fora do chunk de código)? Podemos usar 'in-line code' para exibir um valor simples no documento final. O formato é como o exemplo abaixo, usando o acento grave para começar e terminar, a letra `r` para mostrar que queremos um cálculo do R, e o nome do objeto que queremos inserir. Você se lembra que já criamos o objeto `area` acima com o resultado do cálculo da área do círculo?
...
Na prática, a nossa recomendação é usar opção (2) quando precisa rodar uma linha de código que já existe no seu script, e opção (1) quando precisa rodar temporiaramente uma linha nova, por exemplo para ajudar a inspecionar um objeto.
...
Note que a execução do código por qualquer método gerou o objeto `area` na janela/aba de 'Environment' do RStudio. O Environment armazena todos os objetos que estão disponíveis em modo interativo.
...
2. Cada vez que rodamos o script em que precisamos do conteúdo do pacote (então deixe essa linha dentro do seu script, no início de preferência): 
...
```
````
Note que precisamos de aspas no primeiro caso e não precisamos de aspas no segundo. 
...
Pode usar o atalho `Ctrl+c` para comentar a(s) linha(s) selecionada(a) ou a linha na qual o cursos estiver posicionado.
