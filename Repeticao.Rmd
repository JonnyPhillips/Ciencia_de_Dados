...
O uso de repetição exige uma separação clara dos dois elementos fundamentais de programação em R: **Objetos** (tibbles, vetores, valores únicos), e **Funções** (ações que recebem insumos e geram produtos). Com repetição, normalmente fixamos as funções e ajustamos os objetos, para que os insumos às funções variem, e avaliamos a variação nos produtos finais. A função só percisa ser escrita uma vez, evitando a repetição manual do cálculo.

Podemos gerar uma função que aceita um vetor (uma coluna de nosso tibble), e encaminhar várias colunas para a nossa função. (Revise a introduçao a funções em [Tutorial 4](https://jonnyphillips.github.io/Ciencia_de_Dados/Estatisticas_Resumidas.html) se ajudar). Com o exemplo de calcular amplitude:
...
O uso da função já acelera a análise bastante! E também deixa fácil ajustar o cálculo no futuro. Por exemplo, para arredondar os resultados, é só incoporar uma vez na função, e vai afetar todos os cálculos seguintes:
...
Muito mais simples! Agora em mais ou menos quatro linhas. E note quanto fácil seja ampliar a análise para mais variáveis - é só incluir elas dentro de `select`:
...
# Repitindo a aplicação de funções para vetores fora de um tibble

Observe que a funcionalidade aqui é bem parecida de `summarize(across())` do Tutorial 4, permitindo a aplicação de uma função à várias colunas. É verdade, mais o ganho de `map` é que ela é mais geral e pode ser usado fora do contexto de `mutate` e `summarize`, e fora de um tibble único. Por exemplo, imagine que temos um vetor de centenas de arquivos .csv para abrir em R na pasta do nosso projeto - `map` permite aplicar a função de `read_csv` para *todos* os arquivos em uma linha só (gerando dois arquivos de csv primeiramente para ilustrar):
...
A iteração aqui não é sobre mútliplas colunas, mas sobre cada elemento do vetor 'arquivos'. O resultado 'dados' é uma lista. Dentro da lista existem, separadamente, dois tibbles. Temos duas opções para juntar os dois tibbles em um. Podemos substituir `map_df` para `map`, como já aprendemos acima. Ou podemos manter `map` e seguir com `bind_rows()` que junta tibbles verticalmente:
...
# Repitindo a aplicação de funções para uma lista
...
Uma lista é um conjunto de objetos diversos organizado verticalmente. Não é um tibble com linhas e colunas, listas só tem 'elementos', caixas vazias para guardar qualquer tipo de objeto - valores únicos, tibbles ou outra listas.
...
Para entender o use de `map` com listas, e especificamente com diversos tibbles, vamos separar o banco de dados de `flights` em dois meses, organizar eles em uma lista (com a função `list()`), e aplicar uma função a cada banco (cada tibble). Como exemplo, vamos calcular o atraso médiaopor aeroporto com `summarize()`. 
...
Fácil, sim? Não tem tanta diferença entre `list()`, criando uma lista, e `c()`, criando um vetor: a diferença é simplesmente que cada elemento de `c()` tem que ser um valor único, enquanto `list()` é mais flexível, aceitando qualquer tipo e tamanho de objeto, incluindo tibbles. O resultado seria mais claro e accessível se forçamos a produção de um tibble:

```{r, eval=T}
flights_jan_fev %>% map_df(summarize, dep_delay=mean(dep_delay, na.rm=T))
```

Há apenas um problema com o resultado aqui - não sabemos qual valor descreve qual aeroporto! É importante manter a identificação dos nossos resultados, e seria útil ter uma coluna com o nome do aeroporto dentro do tibble. Isso é possível a partir de um argumento adicional em `map_df`: especificando o nome da coluna no argumento `.id="nome_de_coluna"`
...
Agora, vale a pena resumir os usos de `map`, porque estamos trabalhando num nível mais abstrato e mais flexível, com uma variadade grande de opções:
...
A família de funções de `map` ajudam bastante na repetição porque eles automatizam o processo de substiuição de cada insumo na função, e a organização do resultado. Porém, é uma família nova, e historicamente repetição foi realizada com uma ferramenta mais manual, o '`for` loop'. Recemendamos trabalhar com `map` quando possível para simplicidade, mas é útil e pedagogicamente interessante entender os `for` loops, então incluimos uma introdução rápida na caixa abaixo. 

<div class = "blue">

**Habilidade Básica de Programação: For Loops**

Uma forma alternativa para repetir ações em R é o uso de 'for' loops. Eles são muito comuns em programação, então é bom conhecer, mas se você consiga usar `map`, é normalmente mais eficiente. 
...
3. **A definição do loop mesmo** - `for (i in elementos) { }`. Isso significa que a função dentro das chaves repete para cada valor do vetor elementos, cada vez substituindo `i` na função com o valor de `elementos` atual. 
...
1. Gere três tibbles seperados derivados do banco de dados `flights`, um para cada aeroporto de origem (`origin`). Em seguida, coloque os três tibbles em uma lista única (`list()`), com o nome de cada elemento da lista a sigla do aeroporto. 
...
2. Use `map` para calcular o número de observações (vôos) em cada tibble usando a função `nrow`.

```{r, eval=F, echo=F, tidy=TRUE, code_folding="Mostrar Código"}
flights_por_origem %>% map(nrow)
```

3. Filtre os três bancos de questão 1 para vôos com destino 'SFO' (San Francisco) usando `map`. Quantos vôos (observações) têm cada aeroporto de Nova Iorque para SFO?
...
4. Para cada um dos três bancos de dados de questão 1, calcule a velocidade de cada vôo usando `map`.
...
5. Baseado no seu resultado de questão 4, resume cada um dos três bancos para calcular a média da velocidade dos vôos usando uma função da família `map`. O resultado deve ser um tibble com todos os detalhes apropriados.
...
Todas as aplicações de `map` acima produzem *novos* tibbles ou listas. Mas quando os resultados da aplicação da função cabe com o tamanho do nosso tibble original - por exemplo, cada linha do tibble é um estado e temos um resultado por cada estado - é desejável manter os resultados dentro do tibble original, e apenas adicionar mais uma coluna. 

Neste caso, uma forma muito útil de usar `map` é *dentro* de `mutate`. Isso permite inserir os resultados do cálculo repitido em uma nova coluna em nosso tibble. Veja uma forma interessante de abrir os arquivos .csv - podemos gerar um tibble com a primeira coluna documentando os *nomes dos arquivos*, e depois usar `map` conjunto com `read_csv` para inserir os *dados* de cada arquivo em uma nova coluna. O nosso tibble agora é mais parecido com um armário de arquivos, com cada gaveta bem marcado.  

Note que agora temos que indicar para `map` para qual coluna queremos aplicar a função `read_csv` - o ordem é `map(coluna, função)`. Cada linha reflete um arquivo, e os dados - literalmente os tibbles - ficam 'nested' numa lista-coluna (como descobrimos no tutorial 7).
...
Este procedimento pode ajudar para organizar bases de dados grandes, pois podemos trabalhar com uma tabela simples de resumo, e não múltiplas tabelas inteiras. 

## Repetindo Funções com Tibbles Nested

Qual é o classe da coluna de '`conteudo`' no tibble `dados_nested`? É uma lista, uma lista de tibbles. Então o nosso fluxo de análise não precisa parar aqui. Podemos continuar usando `map` para aplicar uma função para cada tibble separadamente. 

Por exemplo, vamos adicionar uma nova coluna no tibble de resumo com o número de vôos (o número de observações) em cada banco/linha.
...
É comum em análises empíricas executar várias estimativas. Por exemplo, com regressões podemos ajustar o banco de dados para estimar resultados em amostras diversas. Vamos usar este exemplo para aprender como a executar múltiplas regressões em uma linha única de código.
...
Agora temos o resultado básico da regressão em nosso tibble. Provavalmente queremos limpar isso para tornar o resultado principal mais accessível. Por exemplo, podemos aplicar `tidy` com mais uma chamada de `mutate` e `map` para gerar um tibble dos resultados:
...
Na situação em que começamos com um banco de dados inteiro e quer aplicar uma regressão por grupo, em que o grupo é definido por uma variável, é só usar `group_by` e `nest` para preparar o tibble apropriado. Por exemplo, aplicando regresões distintas por aeroporto de origem:
...
Agora, os dois resultados refletam ambos uma amostra (banco de dados) e uma fórmula diferente para cada linha do tibble original (cada aeroporto de origem).
