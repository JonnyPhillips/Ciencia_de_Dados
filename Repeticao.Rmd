...
O uso da função já acelera a análise bastante! E também deixa fácil ajustar o cálculo no futuro. Por exemplo, pata arredondar os resultados, é só incoporar uma vez na função, e vai afetar todos os cálculos seguintes:
...
Muito mais simples! Agora em mais ou menos três linhas. E note quanto fácil seja ampliar para mais variáveis - é só incluir elas dentro de `select`:
...
Observe que a funcionalidade aqui é bem parecida de `summarize_at()`, permitindo a aplicação de uma função à várias colunas. É verdade, mais o ganho de `map` é que ela é mais geral e pode ser usado fora do contexto de `mutate` e `summarize`, e fora de um tibble único. Por exemplo, imagine que temos um vetor de centenas de arquivos .csv para abrir em R na pasta do nosso projeto - `map` permite aplicar a função de `read_csv` para *todos* os arquivos em uma linha só (gerando dois arquivos de csv primeiramente para ilustrar):
...
`map` funciona com vetores ou listas, aplicando a função a cada elemento do vetor/lista. Listas são úteis porque eles podem conter uma variedade de outros objetos - valores únicos, vetores, tibbles, outra listas. Se tivermos vários tibbles, podemos organizar eles em uma lista. 
...
Para entender o use de `map` com listas, e especificamente com diversos tibbles, vamos separar o banco de dados de `flights` em dois meses, organizar eles em uma lista (com a função `list()`), e aplicar uma função a cada banco (cada tibble). Como exemplo, vamos calcular o atraso média por aeroporto com `summarize()`. 
...
Todas as aplicações de `map` acima produzem *novos* tibbles ou listas. Mas quando os resultados da aplicação da função cabe com o tamanho do nosso tibble original - por exemplo, cada linha do tibble é um estado e temos um resultado por cada estado - é desejável manter os resultados dentro do tibble original. 

Nesse caso, uma forma muito útil de usar `map` é *dentro* de `mutate`. Isso permite inserir os resultados do cálculo repitido em uma nova coluna em nosso tibble. Veja uma forma interessante de abrir os arquivos .csv - podemos gerar um tibble com a primeira coluna documentando os *nomes dos arquivos*, e depois usar `map` conjunto com `read_csv` para inserir os *dados* de cada arquivo em uma nova coluna. O nosso tibble agora é mais parecido com um armário de arquivos, com cada gaveta bem marcado.  

Note que agora temos que indicar para `map` para qual coluna queremos aplicar a função `read_csv` - o ordem é `map(coluna, função)`. Cada linha reflete um arquivo, e os dados ficam 'nested' numa lista-coluna (como descobrimos no tutorial 7).
...
Qual é o classe da coluna de `conteudo` no tibble `dados_nested`? É uma lista, uma lista de tibbles. Então o nosso fluxo de análise não precisa parar aqui. Podemos continuar usando `map` para aplicar uma função para cada banco de dados no tibble separadamente. 

Por exemplo, vamos adicionar uma nova coluna com o número de vôos (o número de observações) em cada banco/linha.
...
Agora temos todos os resultados do teste de correlação na coluna `corr`. Para deixar apenas a estimativa central de correlação visível, podemos adatar a função para extrair o valor específico desejado.
...
Agora temos o resultado básico da regressão em nosso tibble. Provavalmente queremos limpar isso para tornar o tresultado principal mais accessível. Por exemplo, podemos aplicar `tidy` com mais uma chamada de `mutate` e `map` para gerar um tibble dos resultados:
...
Na situação em que começamos com um banco de dados inteiro e quer aplicar uma regressão por grupo, em que o grupo é definido por uma variável, é só usar `group_by` e `nest` para preparar o tibble apropriado. Por exemplo, aplicado regresões distintas por aeroporto de origem:
...
Não é apenas o banco de dados que é possível ajustar numa regressão - o `lm` aceita vários argumentos; 'data', mas também 'formula', o modelo da regressão. Isso é comum - funções aceitam vários argumentos e às vezes queremos variar mais que um argumento ao longa da nossa repitição. Por exemplo, eu queria rodar modelos A e B para banco de dados X e Y, modelo C para banco de dados Y e Z, e modelo D para banco de dados Z. 

Operações dessa forma exigem que indicamos para `map` *duas* colunas para encaminhar a nossa função, uma para cada argumento. É simples: verificamos que as duas colunas com os argumentos desejados existem no tibble, usamos `map2` em vez de `map`, e indicamos os nomes das duas colunas no `map2`, no ordem em que eles são esperados pela função que vamos aplicar.
...
Agora, os dois resultados refletam ambos uma amostra e uma fórmula diferente para cada linha do tibble original.

Com as funções de tidyverse básico e a família de `map`, agora você consegue executar quase qualquer tarefa da ciência de dados - são um conjunto de ferramentas muito poderoso. As possibilidades são infinitas- para mais informação e operações mais complexas, veja [o cheatsheet aqui](https://github.com/rstudio/cheatsheets/blob/master/purrr.pdf).
