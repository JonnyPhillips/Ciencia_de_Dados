...
(c) Dos arquivos descompactados, abre os dois bancos para Roraima (RR) em R com os parâmetros apropriados (note que o formato é um pouco diferente dos arquivos no desafio 1). Para detalhes sober as colunas, veja o parte apropriado do documento leiame.pdf nos arquivos.

```{r}
library(tidyverse)
resultados <- read_delim("C:/Users/Jonny/Downloads/votacao_candidato_munzona_2016 (1)/votacao_candidato_munzona_2016_RR.csv",
                   delim=";",
...
(a) Identifique a unidade de análise de cada banco de dados, e lista as variáveis que compõem os identificadores comuns para realizar a junção dos dois bancos. (Cuidado: o número do candidato (`NR_CANDIDATO`) não é único para cada candidato, use uma outra variável).

(b) Use `anti_join` para identificador se há candidatos faltando no banco de dados de resultados. (Bonus: Investigando as colunas do resultado de `anti_join`, pode identificar porque eles não existem no banco de resultados?).

```{r}
candidatos %>% anti_join(resultados, by=c("SQ_CANDIDATO","CD_CARGO","NR_TURNO"))

resultados %>% distinct(SQ_CANDIDATO, CD_CARGO, NR_TURNO)

candidatos %>% distinct(SQ_CANDIDATO, CD_CARGO, NR_TURNO)

candidatos %>% distinct(SQ_CANDIDATO)

candidatos %>% group_by(SQ_CANDIDATO) %>% tally() %>% arrange(-n)

```

(c) Execute um join do tipo apropriado para uma análise comparando as caractéristicas dos candidatos e os resultados eleitorais. 

```{r}
combinado <- resultados %>% left_join(candidatos, by=c("SQ_CANDIDATO","CD_CARGO","NR_TURNO"))
