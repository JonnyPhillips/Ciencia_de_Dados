...
2. Use a funcionalidade da família de `map` para aplicar a função `read_municipality` para os cinco estados seguintes em uma linha única de código: SP, RJ, MT, RS e RN (todos para o ano de 2018). 
...
3. Baixe, unzip e abre em R o arquivo da população paulista em 2010 [do site do IBGE](https://www.ibge.gov.br/estatisticas/downloads-estatisticas.html), 'Censos' -> 'Censo_Demografico_2010' -> 'resultados' -> 'total_populacao_sao_paulo.zip'. 
...
4. Queremos mapear dados da população por município. Identifique o chave apropriado, e cruze o banco de população com o banco das fronteiras dos municípios de SP. 
...
5. Usando o seu banco de dados de Questão 5, calcule a proporção da população urbana na população total em cada município e visualize um mapa bem-formatado dessa taxa por município em 2010. Aplique uma escala de cores desejada.
...
**Testes Estatístos e Regressões**

6. Faça um teste de shapiro para avaliar se a taxa urbana municipal é distribuida de forma normal.

```{r}
muns_SP %>% pull(taxa_urbana) %>% shapiro.test()
```

7. Execute uma regressão para avaliar se a taxa urbana (a variável dependente) é associada com a população total do município (a variável independente). Apresente o resultado numa tabela bem-formatada.
...
8. Mostre um gráfico do efeito marginal (o coeficiente) da variável da população na regressão em Questão 8 e o intervalo de confiança da coeficiente.
...
9. Use [o link aqui](https://escriba.camara.leg.br/escriba-servicosweb/pdf/59638) para acessar em R um PDF da discussão na Camara dos Deputados no dia 21 de Maio de 2020. Transforme o PDF em texto simples.
...
10. Precisamos processar e preparar o texto para a análise. Segue os seguintes passos:
    ...
    c. Em seguida, `unnest()` os dados para que cada fala de cada deputado é uma linha separada no tibble.
...
11. Agora, com o tibble produzido em Questão 17, vamos desaggregar e padronizar os discursos:
