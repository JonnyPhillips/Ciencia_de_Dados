...
Se quisermos as observações em ordem de ano e valor:
...
Com os nossos dados ordenados, podemos implementar várias técnicas de análise de Time Series (séries temporais). Por exemplo, podemos calcular, para cada observação, o valor de uma variável no período anterior usando `lag()`. Note que é *essencial* que os nossos dados já estão ordenados com `arrange()` antes de executar o `lag`. Dado que estamos construindo uma nova variável, também é necessário usar `lag` dentro de `mutate`:
