...
<aside>
```{r, echo=T, eval=F, tidy=TRUE, tidy.opts=list(width.cutoff=30)}
...
<aside>
```{r, echo=T, eval=F, tidy=TRUE, tidy.opts=list(width.cutoff=20)}
flights <- flights %>% 
...
O código acima mostra como a criar variáveis de tipo factor num novo banco de dados. Mais frequentemente, já temos um banco de dados com uma variável de tipo caractere, e queremos transformá-lo em factor. Vamos criar o mesmo banco de dados de novo, mas sem especificar o mês como factor, e depois aplica a transformação dentro de mutatue.
...
Note que aqui não temos que especificar os 'levels' do factor - o R use os valores atuais para preencher os levels. Podemos fazer assim, mas lembre-se que os levels não incluem todos os meses, que pode gerar problemas no futuro se quisermos adicionar mais dados. 
...
<aside>
```{r, echo=T, eval=F, tidy=TRUE, tidy.opts=list(width.cutoff=25)}
flights %>% 
  mutate(origin = 
           fct_recode( origin,  
                                     "Newark"="EWR", 
...
O prazo para entregar Desafio 1 por email à minha conta é **14h, 27/03/2020**, antes da próxima aula. Por favor entregue (i) o arquivo .Rmd, e (ii) o arquivo .html.
</div>