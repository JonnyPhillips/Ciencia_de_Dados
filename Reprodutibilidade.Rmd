...
- Clique 'Commit' na aba de Git e vai abrir uma caixa que mostra todas as diferenças entre o nosso código local e o código do arquivo no github online: aditivos em verde e remoções em vermelha. A única coisa que temos que fazer nessa caixa é adicionar uma mensagem que descreve o conteúdo da mudança feita, ex. "Adicionar gráfico de atrasos", e clique em `Commit`. Quando terminado, feche as caixas. A aba de Git deve mostrar a notícia de "Your branch is ahead of 'origin/master' by 1 commit". Isso significa que o seu novo script foi cadastrado no fluxo de trabalho do projeto (o controle de versão), mas ainda não foi sincronizado online. 
...
Agora vá para o repositório na sua conta no Github online, atualize a página, e confira que o novo arquivo 'teste.Rmd' aparece. (É possível que tem que aguardar alguns minutos para ele aparecer, mas normalmente é imediato). Se entrar no link de '1 commit' na linha azul na página do seu repositório, pode ver todos os commits anteriores, e os detalhes de mudanças de código em cada um. Ou pode clicar em cada arquivo e depois 'History' (história) rastrear todas as mudanças anteriores. 

Parabéns, agora o seu trabalho está seguro com um backup online, documentado com a história de versões rastreável, e accessível por seus colegas!
...
Se você tenha uma conta paga, ou uma [conta estudantil](https://education.github.com/pack), pode tornar a sua conta privada e adicionar apenas os colaboradores desejados - em Github vá para Settings -> Make Private.
...
2. Copiar o seu script de Exercício 1 acima (o .Rnw) para a pasta local do seu projeto novo criado no passo anterior. 
...
4. Usando a aba de Git, `Commit` a versão atualizada do script Rnw com uma descrição apropriada.

5. `Push` as mudanças para o seu repositório de Github online. Verifique que o novo arquivo está atualizado no repositório da sua conta de Github.
...
O que vai acontecer se mandamos o nosso script de R Markdown para uma colega? Vai funcionar? Será reprodutível? Frequentemente não. Mesmo que eles tem exatamente os mesmos arquivos e usa a mesma versão de R e RStudio, não podemos garantir que eles terão acesso às mesmas funções que usamos para fazer nosso script. Lembre-se que tivemos que instalar cada uma das nossas bibliotecas (`install.packages()`) mas essas linhas de instalação não ficam em nosso script - se o receptor não tiver feito o mesmo ou tiver uma versão diferente de uma biblioteca (eles estão atualizadas frequentemente), nosso código vai quebrar.

Para resolver isso, podemos usar o pacote `renv` para garantir reprodutibilidade de bibliotecas. O `renv` salva as bibliotecas na pasta do seu projeto, e documenta as versões que estão sendo usadas. No modo interativo, rodamos a função `init()` uma vez quando começamos um novo projeto. Em seguida, quando você quer gravar o status das suas bibliotecas (normalmente depois de abrir novas bibliotecas com `library()`) pode usar `snapshot()`. 
...
Muita coisa, mas em combinação, o R, Latex, Git e Renv permitem relatórios reproduzíveis que garante a história e integridade do seu relatório ou artigo profissional. 
